...
from vaxport.skill_validator import SkillValidator
from vaxport.semantic_memory import SemanticMemory
...
6. **使用 deep_research_collect 进行 Deep Research 聚合采集**（复杂多维度任务优先使用）

工作流：查询数据 → 统计分析 → 异常检测 → 生成报告（按需）

## Deep Research 使用指引

对于复杂多维度分析任务（如 APQR 年度回顾、多产品偏差调查、多参数联合分析），
优先使用 `deep_research_collect` 工具替代逐条 query_* 串行查询：

**何时使用**：任务涉及 3+ 张表或多维度分析时
**参数格式**：
```json
{
  "tables_needed": [
    {"schema": "analog_quality", "table": "deviations", "why": "偏差回顾", "key_filter": "2024年", "aggregate_hint": "GROUP BY severity, status → COUNT"},
    {"schema": "analog_quality", "table": "final_product_qc", "why": "CQA趋势", "key_filter": "产品=PEDV", "aggregate_hint": "按月份 AVG(potency), STDDEV(potency)}
  ],
  "output_sections": ["偏差总览", "趋势分析", "结论"],
  "task_id": "可选，自动生成"
}
```

**Deep Research 返回聚合摘要而非原始数据**，分析维度被数据范围锁死。
如需特定表的详细数据，后续用 query_* 精确查询1-2次补充。
**简单任务不需要 Deep Research**，直接用 query_* 即可。
...
class Orchestrator:
    ...
    TOOL_FILTERS = {
        ...
        "analyze_reporter": {"query_", "generate_chart", "detect_anomaly", "generate_report", "run_statistics", "deep_research_collect"} | BASIC_TOOLS,
    ...
    def run(self, user_query: str, callbacks: ProgressCallbacks = None,
            ...
            cancel_event=None, skill_enabled: bool = True) -> dict:
        ...
        # SKILL 匹配与注入（Layer 1 防御）
        matched_skill = None
        skill_prompt = None
        skill_validator = None
        if skill_enabled:
            matched_skill = self._skill_engine.match_skill(user_query)
            if matched_skill:
                skill_prompt = matched_skill.to_prompt_section()
                if matched_skill.checklist:
                    skill_validator = SkillValidator(matched_skill.checklist)
                logger.info(f"SKILL matched: {matched_skill.name}")

        # 语义召回注入（路由后、Agent执行前）
        semantic_context = ""
        if self._semantic_memory:
            semantic_context = self._semantic_memory.build_context_section(user_query)
            if semantic_context:
                logger.info("SemanticMemory: 注入相似历史案例")

        # 合并 SKILL prompt 和语义召回
        combined_prompt = ""
        if skill_prompt:
            combined_prompt = skill_prompt
        if semantic_context:
            combined_prompt = (combined_prompt + "\n\n" + semantic_context) if combined_prompt else semantic_context
        ...
        result = agent.run(effective_query, callbacks=callbacks, plan_mode=plan_mode,
                           ...
                           extra_system_prompt=combined_prompt)
        result["agent_type"] = route["intent"]
        result["agent_chain"] = [route["intent"]]
        result["task_id"] = task_id  # 暴露task_id供前端反馈使用

        # SKILL 输出验证（Layer 3 防御）
        if skill_validator and matched_skill:
            tool_calls = result.get("tool_calls", [])
            validation = skill_validator.validate(result.get("answer", ""), tool_calls)
            result["skill_validation"] = {
                "skill": matched_skill.name,
                "passed": validation.passed,
                "score": validation.score,
                "issues": [{"level": i.level, "message": i.message} for i in validation.issues],
            }

        # 语义记忆存储（成功任务后）
        if self._semantic_memory and result.get("answer") and not result.get("error"):
            try:
                # 从 tool_calls 提取使用过的表名
                tool_calls = result.get("tool_calls", [])
                tables_used = []
                for tc in tool_calls:
                    ...
                    if name.startswith("query_"):
                        # 从工具名提取 schema.table
                        parts = name.replace("query_", "").split("_")
                        if len(parts) >= 2:
                            tables_used.append(f"{parts[0]}.{parts[1]}")
                self._semantic_memory.store_analysis_case(
                    task_id=task_id,
                    query=user_query,
                    conclusion=result.get("answer", "")[:500],
